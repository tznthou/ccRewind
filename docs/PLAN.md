...
- [x] A/C/B1/B2 實作完成，隨 v1.18.0 發布（2026-07-07）。實作期間兩個裁決點：(1) B1 的 `isCompactSummary` 實測 2366 個真實 session 從未出現、`isSidechain` 100% 侷限在 subagent 檔案內（跟既有 `subagent_sessions` 機制重疊）——子超裁決仍照原計畫全做，為 schema 演進留活口；(2) B2 上線前用真實 markdown-tool session 驗證時發現原「1-hop 有無子節點」判定法漏抓棄用分支（常帶 1 筆 bookkeeping entry 才斷鏈），改為比較分支延伸深度比例（<10% 門檻）才正確捕捉真實案例
