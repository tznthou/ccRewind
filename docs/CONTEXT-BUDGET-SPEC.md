...
#### API 計費/追蹤欄位（2025-04 ~ 2026-02 陸續新增，ccRewind 尚未解析）

以下欄位是 Anthropic Messages API response 的擴充，用於 API 使用者追蹤計費與推論細節。Claude Code 忠實地將 API response 寫入 JSONL，因此 ccRewind 可見但目前未解析。

| 欄位 | 型別 | 新增時間 | 說明 |
|------|------|----------|------|
| `service_tier` | string | 原始欄位 | `"standard"` 或其他 |
| `server_tool_use` | object | 2025-05 | 伺服器端工具呼叫次數，含 `web_search_requests` / `web_fetch_requests`（每 1,000 次搜尋 $10） |
| `cache_creation` | object | 2025-04 beta / 2025-08 GA | cache 寫入的 TTL 拆分：`ephemeral_5m_input_tokens`（5 分鐘 1.25x）/ `ephemeral_1h_input_tokens`（1 小時 2x）。加總等於 top-level `cache_creation_input_tokens` |
| `speed` | string | 2026-02 | 推論速度：`"standard"` / `"fast"`。Fast mode 產出速度 ~2.5x，定價 6x |
| `inference_geo` | string | 2026-02 | 推論地區：`"us"` / `"global"` / `""`（未指定）。US-only 定價 1.1x |
| `iterations` | array | 未公開文件 | Claude Code harness 內部追蹤，每元素為一次 API round-trip 的 token 拆分。目前觀察均為單元素陣列 |

> **Parser 影響評估**（2026-05-26 盤點）：核心 token 計算不受影響——`cache_creation_input_tokens` 已是 5m + 1h 的總和。`cache_creation` 拆分在未來若要呈現 cache 策略成本差異時才需解析。`server_tool_use` / `speed` / `inference_geo` 待有實際非零資料再評估是否進 DB。
