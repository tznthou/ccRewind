...
**為什麼不做 LLM**：ccRewind 屬 ccFamily 三件套，**整體哲學為 rule-based + zero API cost**。AI 跨 session 記憶由姊妹專案 [ccRecall](https://github.com/tznthou/ccRecall) 承接（No LLM dependency）；更深入的 ADR 推理由規劃中的 ccReason plugin 承接（獨立 npm 包但讀 ccRecall schema）。ccRewind 自身專注把 heuristic 摘要推到天花板。
...
## Phase 7.7 ✅ Token Budget i18n + plan detection 修正（v1.12.1）

**目標**：v1.12.0 的 Dashboard i18n 把對話介面以外的最後一塊 hardcoded zh-TW 處理掉之後，剩下 Token Budget 子系統還有兩條雙語 hold-out（6 個 chart components + insightEngine），且 plan detection 漏寫導致 226K context 在 1M 模型下被誤報為「113% of 200K」。此版本一次處理 i18n 收尾、plan 偵測修正、panel 錯誤訊息 fail-close。

### 7.7-A. Token Budget panel internationalization（commit 492bf2b）

- 6 個 chart components（`TokenSummaryCard` / `ContextGrowthChart` / `TokenBreakdown` / `CostHeatBar` / `InsightsPanel` / `TokenBudgetPanel`）+ `insightEngine` 是最後 hardcoded-bilingual hold-out：每個 Insight title / body 都是 static zh-TW 字串靠 runtime locale switch 切
- engine 改回傳 `Insight { data: InsightData }` discriminated union（7 種 insight type + `SpikeCause` sub-union），engine 本體變得 i18n-agnostic；UI 端加 `insightMessages.ts` 做 `InsightData → MessageKey` 映射
- 43 個新 `tokenBudget.*` keys，zh-TW + en lockstep（`satisfies MessageCatalog` 編譯期強制）
- `insightMessages.test.ts`：14 sample × 2 locale smoke + 14 explicit mapping-correctness assertions，新增 insight type 漏接 message key 會直接讓測試紅，而不是靜默 render 空字串

### 7.7-B. 1M plan detection（commit 768d95e）

- **問題**：`assessContextLimit` 是一串 `if` cascade，沒做 plan 偵測——200K 到 800K 之間的 `contextTotal` 全部落入 200K branch，render 出「Context at 113% of 200K limit (226.8K)」這種物理上不可能的訊息（200K context 模型本來就會在 > 200K 時拒絕請求；能看到 226K 就證明 session 跑在 1M context model 上）
- **修正**：新增 `detectContextPlan(turns)`，任一 `contextTotal > 200K` 即回傳 `'1m'`，否則 `'200k'`；`assessContextLimit` 取對應閾值 band；`ContextGrowthChart` 的 reference-line toggle 預設值改用偵測結果，圖表與 Insights panel 維持一致
- **前提**：one plan-class per session——current architecture 下成立（subagent turns 在獨立 JSONL；mid-session `/model` 切跨 plan 罕見），inline 註解記錄此假設
- 5 條 regression tests，包含原始 226K screenshot case

### 7.7-C. Panel error fail-close（commit 7f014fa）
...
| 語意搜尋 | FTS5 夠用就不投入；ccFamily 整體不走 LLM 路線 |
