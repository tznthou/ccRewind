...
這些 entries 共享相同 `requestId` / `message.id`，每筆都帶相同的 `usage` 欄位。
ccRewind 在 indexer 層以 `deduplicateTokensByRequestId()` 處理：同一 `requestId` 的前 N-1 筆
token 欄位清零，只保留最後一筆，避免 input token 膨脹（v1.7.2 修正）。
...
ccRewind 自動掃描並索引 subagent，儲存於獨立 `subagent_sessions` 表（migration v12），
以 parent-child linkage 關聯主 session。主視圖查詢透過 `EXCLUDE_SUBAGENTS` 排除 subagent，
前端以 chips 導覽 + breadcrumb 提供 subagent 瀏覽入口（v1.8.0）。
