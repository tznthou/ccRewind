...
describe('resolveBlockedBy', () => {
  it('查得到的 id 帶出對應 subject', () => {
    expect(resolveBlockedBy(subjectById, ['2'])).toEqual<BlockedByRef[]>([
      { id: '2', subject: '寫 parser' },
    ])
  })

  it('查不到的 id（指向清單外的 task）subject 為 null', () => {
    expect(resolveBlockedBy(subjectById, ['99'])).toEqual<BlockedByRef[]>([
      { id: '99', subject: null },
    ])
  })

  it('混合查得到與查不到時，保持 blockedBy 的輸入順序', () => {
    expect(resolveBlockedBy(subjectById, ['2', '99', '1'])).toEqual<BlockedByRef[]>([
      { id: '2', subject: '寫 parser' },
      { id: '99', subject: null },
  ...
  it('subject 為空字串的 task 仍視為存在（subject 是 \'\' 不是 null）', () => {
    ...
    expect(resolveBlockedBy(subjectById, ['3'])).toEqual<BlockedByRef[]>([
      { id: '3', subject: '' },
    ])
  })

  it('空 blockedBy 回傳空陣列', () => {
    expect(resolveBlockedBy(subjectById, [])).toEqual([])
  })
})
