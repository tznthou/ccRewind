...
describe('migration v27: strip payloads the UI cannot display from message_content', () => {
  const SIGNATURE = 'EosnCkYI'.repeat(400)
  const BASE64 = 'iVBORw0KGgo'.repeat(500)

  const thinkingRow = JSON.stringify([
    { type: 'thinking', thinking: 'check the schema first', signature: SIGNATURE },
    { type: 'text', text: 'done' },
  ])
  const omittedThinkingRow = JSON.stringify([{ type: 'thinking', thinking: '', signature: SIGNATURE }])
  // v1.16.0 之前索引、原檔已被清掉的 session：頂層圖片的 base64 還在
  const legacyImageRow = JSON.stringify([
    { type: 'text', text: 'look at this' },
    { type: 'image', source: { type: 'base64', media_type: 'image/png', data: BASE64 } },
  ])
  const nestedImageRow = JSON.stringify([
    {
      type: 'tool_result',
      tool_use_id: 't1',
  ...
  function storedRows(): Array<{ content_text: string | null; content_json: string | null }> {
    return db.rawAll(
      `SELECT m.content_text, mc.content_json FROM messages m
       LEFT JOIN message_content mc ON mc.message_id = m.id
       WHERE m.session_id = 'v27-seed' ORDER BY m.sequence`,
    )
  }
  ...
  it('replaces thinking signatures with a marker and keeps the thinking text', () => {
    seedAndUpgradeFromV26()
    const stored = storedRows()
    expect(JSON.parse(stored[0].content_json!)).toEqual([
      { type: 'thinking', thinking: 'check the schema first', signature: '[signature-stripped]' },
      { type: 'text', text: 'done' },
    ])
    // omitted thinking 的 signature 仍是非空字串，UI 照樣能說明「這段思考被 API 省略」
    expect(JSON.parse(stored[1].content_json!)).toEqual([
      { type: 'thinking', thinking: '', signature: '[signature-stripped]' },
    ])
  })

  it('strips base64 from top-level images, images inside tool results, and PDFs', () => {
    seedAndUpgradeFromV26()
    const stored = storedRows()
    expect(JSON.parse(stored[2].content_json!)[1].source).toEqual(
      { type: 'base64', media_type: 'image/png', data: '[base64-stripped]' },
    )
    expect(JSON.parse(stored[3].content_json!)[0].content[0].source).toEqual(
      { type: 'base64', media_type: 'image/jpeg', data: '[base64-stripped]' },
    )
    expect(JSON.parse(stored[4].content_json!)[0].source).toEqual(
      { type: 'base64', media_type: 'application/pdf', data: '[base64-stripped]' },
    )
  })

  it('leaves rows without such payloads byte-identical, including ones it cannot parse', () => {
    seedAndUpgradeFromV26()
    const stored = storedRows()
