...
- **「浪費偵測」這個名字在文件裡留了太久**：它曾經是這個功能的正式名稱，v1.12.0（[#19](https://github.com/tznthou/ccRewind/pull/19)）已更名為「未收尾的對話」，但那次更名只動前端——IPC channel `stats:waste` 與 `WasteSession` 型別刻意保留在邊界上，說明文件也一直沒跟上，於是舊名在文件裡多留了三個多月。另外這個功能並沒有任何判定「高 token」的門檻：實際行為是取沒有 commit/test 結果的 session，依 token 用量由高到低列出前 20 筆。文件已改回與介面一致
