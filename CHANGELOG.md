...
- **圖片、PDF 區塊的 base64 資料與思考簽章改存標記，升級時既有資料一併處理**（[#119](https://github.com/tznthou/ccRewind/pull/119)）：這些資料 app 從不顯示成圖片或 PDF，簽章也從不顯示，卻佔去索引很大一塊。前一版只剝除訊息最外層的圖片，工具結果裡的圖片、PDF 與思考簽章都照存；較舊的索引也可能還留著最外層圖片的資料（維護者的索引裡有 286 張，都在 2026 年 3 到 5 月開始的 session）。在維護者自己 1,894 MB 的索引裡，簽章與 base64 加起來 577 MB，佔全部訊息內容（1,218 MB）的 47%。現在訊息最外層、以及工具結果內容裡的圖片與 PDF 區塊保留原本的欄位（類型、media type 等），只把 base64 資料換成 `[base64-stripped]`；thinking 的文字照舊保留，非空的簽章換成 `[signature-stripped]`（空的維持空的），被 API 省略的思考照樣會標示出來。這次的剝除不動工具呼叫的參數，也不動 parser 不認得的區塊；升級時 JSON 無法解析或無法重新序列化的資料列會原樣保留，寫入資料庫失敗則整批回滾、下次啟動再試（維護者的索引處理後，base64 與簽章都沒有殘留）
  - ⚠️ **被換掉的資料無法從標記還原**：原始 JSONL 已經不在的 session，索引可能就是你手上唯一的一份。想留住這些圖片的話，升級前先在 app 關閉時複製一份 `~/.ccrewind/index.db`
  - **升級後第一次開啟要先等資料處理完**：處理會在視窗出現前跑完，時間依資料量與電腦而異，維護者 1.9 GB 的索引實測處理約 8 秒；處理成功之後就不會再跑
  - **升級不會自動縮小資料庫檔案**：想回收空間，到儲存管理頁按「壓縮資料庫」；維護者的索引壓縮後從 1,894 MB 降到 1,274 MB

### Fixed

- **工具結果裡的 base64 圖片改顯示標記，不再是一長串 base64**（[#119](https://github.com/tznthou/ccRewind/pull/119)）：前一版沒有剝除工具結果內容裡的圖片區塊（例如讀取圖片檔、MCP 截圖回傳的），展開含這種圖片的工具輸出時會看到整串 base64 文字。現在這些區塊的 base64 資料跟最外層的圖片一樣換成標記

## [1.23.0] - 2026-09-25

### Added

- **排除規則可以選擇保留既有資料，只停止之後的索引**（[#117](https://github.com/tznthou/ccRewind/pull/117)）：排除原本一次做兩件事——刪掉符合的 session，並擋住它們之後再被索引——而且沒有別的選擇，想要「以後別再索引這個專案，但已經索引的留著」做不到，按下去就刪了。確認對話框現在先問怎麼處理既有資料：
  - **保留資料，停止之後的索引**（預設）：規則建立當下已在索引裡的 session 保留，而且照常更新（對話繼續寫入、重新索引都照舊）；之後才出現的 session 不再索引。移除規則即可撤銷它的阻擋：被它擋下的 session 只要原始 JSONL 還在、也沒有被其他規則擋住，下次索引就會補進來
  - **刪除資料，並停止之後的索引**：原本的行為。影響比例、超過一半的紅色警告與「我了解此操作不可復原」勾選，現在只在這個選項出現

  保留資料的規則會在建立當下記下索引裡已有哪些 session，只管這份名單以外的——這是當下記錄而不是事後推斷，所以別處的刪除觸發保留的 session 重新索引時，它不會因此被誤擋。規則清單會標出每條規則屬於哪一種；移除「已刪除資料」的規則前，會先說明刪掉的 session 只有原始 JSONL 還在的才回得來。主程序套用規則時必須明確帶上處理方式，缺漏或不認得的值一律拒絕，不會預設成刪除。⚠️ **升級前建立的規則一律標為「已刪除資料」**：在此之前建立規則的唯一路徑就是刪除，這是事實而非推測

### Changed

- **依日期範圍排除在沒有符合的 session 時也能建立規則**（[#117](https://github.com/tznthou/ccRewind/pull/117)）：以前這種情況下按鈕是停用的；現在可以送出，對話框只開放「保留資料」一項，可用來預先擋住日期落在某個範圍、而之後才出現的 session。表單的即時預覽也從「將刪除」改為「符合」，因為要不要刪是在對話框裡選的
- **相依套件更新**：`@tanstack/react-virtual` 升至 3.14.13（[#116](https://github.com/tznthou/ccRewind/pull/116)）
...
- **效率趨勢標成「Tokens/輪」，但它算的不是輪次**：分母是 session 索引後保留的 JSONL 記錄數，而那份記錄裡有整整一半是 `attachment`、`progress`、`permission-mode` 這類狀態列，不是對話。在 674,569 列的索引上實測，真正的使用者發話只有 17,501 筆——把記錄數當成輪次，分母膨脹 38.5 倍。效率趨勢圖的提示文字改為「Tokens/則」、專案健康度卡片上的單位改為 `/msg`，說明文件寫明是「每則 JSONL 記錄的平均 token」。**數值本身沒有變**。新標籤也不是完全精確的說法——分母數的仍是 JSONL 記錄而非對話訊息，只是不再把它說成「輪次」
- **「浪費偵測」這個名字在文件裡留了太久**：它曾經是這個功能的正式名稱，v1.12.0（[#19](https://github.com/tznthou/ccRewind/pull/19)）已更名為「未收尾的對話」，但那次更名只動前端——IPC channel `stats:waste` 與 `WasteSession` 型別刻意保留在邊界上。當時的版本說明寫「兩個名字的不對稱已文檔化，且只存在於 IPC seam」，這句低估了範圍：雙語 README、兩份使用指南與入門文件共五份也都留著舊名，一留將近三個月。另外這個功能並沒有任何判定「高 token」的門檻：實際行為是取沒有 commit/test 結果的 session，依 token 用量由高到低排序，面板顯示前 10 筆。文件已改回與介面一致
- **檔案反向索引的說明漏掉第四種操作**：Glob 與 Grep 會記為 `discovery`，而文件只列了 read/edit/write。另外檔案索引不會從 Bash 指令的內容裡取出檔案路徑，這點原本沒有寫明
...
- **一個孤兒 subagent 會中斷整輪索引**：subagent 的 parent 不在 session 表裡時，外鍵約束直接拋出——中斷的不是那一個 session，是整輪索引。有兩條路會留下這個狀態：resume 產生的 session 內容被 UUID 去重成空、以及主檔解析失敗。這兩種情況下 subagent 的 JSONL 都是獨立、沒被去重過的檔案，內容不會因為主檔重複或損壞而失去價值，所以現在補一列僅含中繼資料的 parent 把它接住，而不是連帶丟掉。使用者主動設定的排除規則仍然優先：被規則涵蓋的 parent 不補寫，它的 subagent 也一併留在外面
- **掃描根目錄消失時，整個索引庫會被標成已封存**：外接磁碟卸載、掛載點掉線，掃描結果與「使用者刪光了所有對話」在觀測上一模一樣，處置卻相反。現在掃到 0 筆而資料庫裡有記錄時，會先確認根目錄還在不在，不在就跳過封存，並留下記錄說明原因
- **排除規則刪掉來源之後，replay session 的內容永久讀不回來**：resume 產生的 session 會把前一輪的訊息 replay 進自己的檔案，索引時整批撞上原 session 的 UUID 被去重，自己只留一列中繼資料。原 session 被排除規則硬刪後，那些訊息其實可以重新索引回這個 session 名下——但它的檔案時間不會因為別人被刪而改變、摘要版本又是最新，永遠排不進佇列。結果是磁碟上檔案還在，資料庫卻再也讀不到那段對話
- **型別檢查形同虛設**：`tsc --noEmit` 搭配 solution-style 的 tsconfig 完全不會走 references，等於零檢查；而 `tests/` 與根目錄的建構設定檔本來就不在任何一份 tsconfig 的涵蓋範圍內。實測把一行型別錯誤寫進 `vitest.config.ts`，補上之前照樣通過。現在三份設定各自獨立檢查
- **孤兒 subagent 記錄不會被封存**：來源檔案已經不存在的 subagent 此前會一直停留在活躍狀態

### Changed

- **資料庫 migrations 抽成獨立模組**，與主資料庫程式分離
- **新增流量快照排程**：GitHub 的流量統計只保留 14 天滾動視窗，過期即永久消失。改為每日快照到獨立分支保存。referrer 與路徑的快照以完整時間戳為鍵而非日期——它們拿到的是「滾動 14 天聚合的前 10 名」，同一天兩次執行不是同一次量測，後者也不是前者的超集
- **README 改以追溯決策軌跡開場**，取代原本的佈景主題展示
- **相依套件更新**：Node.js 22.23.2、TypeScript 生態、其他安全性修補

## [1.21.0] - 2026-08-01

### Added

- **索引跳過的項目會顯示在側邊欄**：解析失敗的 session、subagent 或 task 檔此前是靜默跳過的——那筆資料沒進資料庫，使用者不知道，我們自己也不知道。對一個宣稱「一個位元組都不會動」的工具來說，安靜地漏掉整個 session 是說話不算話。現在每一次跳過都會記下是哪一個項目，數量則直接顯示在同步狀態旁（以錯誤色呈現，不用比較柔和的顏色帶過）。涵蓋 session 解析、subagent 掃描與解析、task 掃描、超大 task 檔與無法解析的 task 檔

### Fixed

- **一筆結構異常的訊息會中斷整輪索引**：摘要引擎過去只確認 `contentJson` 是合法 JSON，沒確認解析出來是不是陣列。`{}` 或數字這類合法 JSON 過得了解析，接著在迭代時直接拋出 `TypeError`，讓整個索引程序在完成前中斷——不是那一則訊息失敗，是排在後面的所有 session 都索引不了
- **列表選項的 id 會互相碰撞**：清理 HTML id 時所有非法字元一律換成底線，於是 `a b`、`a#b`、`a_b` 全都塌成同一個 id。`aria-activedescendant` 因此可能指向錯誤的項目，螢幕閱讀器唸出來的與實際選取的不是同一列。改為單射編碼，不同輸入必定得到不同 id
- **鍵盤導覽的索引沒有下界保護**：`setActiveIndex` 是對外開放的介面，傳入負值會讓項目查找得到 `undefined`，後續取 id 時直接拋錯
- **掃描層的失敗不再無聲無息**：檔案掃描器內部會把 `readdir` / `stat` 的錯誤轉成空陣列，權限問題、掛載點消失、檔案讀取失敗因此完全觀察不到，上層的錯誤處理永遠等不到它們。現在除了「目錄不存在」這個常態情況之外都會留下記錄
- **介面殘留的英文字串**：對話氣泡的角色標籤（`User` / `Assistant`）、Session 清單的排序按鈕（`Time` / `Tokens`）與檔案數量、專案排行榜的空狀態，切到中文介面時仍顯示英文。同時新增自動掃描機制，未走翻譯目錄的字串會讓測試失敗

### Security

- **記錄檔注入**：索引過程的記錄會直接嵌入檔名與路徑，而檔名在 macOS 與 Linux 上可以合法含有換行與終端機控制序列。任何能寫入家目錄的東西都可以藉此偽造一行看似由程式輸出的記錄（諷刺的是偽造內容正好可以是「什麼都沒漏」），或對終端機送出控制序列——後者在有漏洞的終端機模擬器上出現過遠端執行的案例。所有來自檔案系統的值在寫入記錄前都會先逃逸
- **路徑穿越**：session 代號取自檔名去掉副檔名，一個名為 `...jsonl` 的檔案剛好會得到 `..`，而 task 掃描直接把它接到路徑上。結果是任何能在 `~/.claude/projects/` 底下放檔案的東西，都能讓索引器改去讀取 `~/.claude/` 本身的 `*.json`（`settings.json` 就在那裡）並寫進索引資料庫。此程式仍然完全不寫入 `~/.claude/`，這裡收緊的是它願意讀取的範圍

### Changed

- **測試基建**：鍵盤導覽的狀態機抽成純函式後補上測試（前述兩個缺陷即由此浮現）；新增覆蓋率報告（目前總計 46.33%，主程序 87.18%）；索引相關測試改為使用隔離的暫存目錄，不再讀取真實的 `~/.claude/tasks/`。測試總數由 583 增至 641
...
- 依賴更新：eslint 10.7.0、typescript-eslint 8.64.0、globals 17.7.0、pnpm 11.13.1、Node.js 22.23.1、@tanstack/react-virtual 3.14.6 及其他 patch 修補
