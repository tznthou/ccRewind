...
No LLM calls. No context injection. No RAG. Session summaries and tags are generated by pure heuristics — zero API cost.
...
| **Session Auto-Summary** | Heuristic-generated intent summary, auto-tags (bug-fix, refactor, etc.), touched files list, and tool usage stats |
...
- **Auto-tags**: Inferred from conversation keywords — `bug-fix`, `refactor`, `testing`, `deployment`, `auth`, `ui`, `docs`, `config`
...
- **Messages** (default): Searches message content. Results are grouped by session. Each result has a ▸ button that expands to show 2 surrounding messages as context preview — judge relevance without navigating away
...
| Vitest 3 | Test framework | 118 tests, run through Electron |
...
- **No LLM calls** — summaries and tags are generated by pure heuristics, zero API cost, zero privacy risk
