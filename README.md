...
| **統計儀表板** | 跨 session 分析：使用趨勢（雙軸面積圖）、專案排名、工具分佈（甜甜圈圖）、標籤分佈、24 小時工作模式熱力圖，全域/專案篩選 + 日期範圍 |
...
點擊標題列的 Dashboard 圖示（四格方塊）切換到統計視圖，提供五個跨 Session 分析面板：

- **Usage Trend**：雙軸面積圖，左軸 session 數 / 右軸 token 消耗，支援 7D / 30D / 90D / All 切換
- **Project Activity**：專案排名（依 session 數排序），顯示 token 總量和比例條
...
graph TB
    ...
    subgraph Renderer Process
        ...
        DB_UI[Dashboard<br>使用趨勢 · 專案排名 · 工具分佈<br>標籤分佈 · 工作模式熱力圖]
...
| Vitest 3 | 測試框架 | 172 個測試，透過 Electron 執行 |
...
│   │   │   ├── Dashboard/     # 統計儀表板：趨勢圖、專案排名、工具/標籤分佈、工作模式
...
│   │   ├── utils/             # formatTokens, formatTime, formatDuration, pathDisplay
│   │   └── context/           # AppContext + ThemeContext（主題持久化）
│   └── shared/
│       └── types.ts           # 主程序與渲染程序共用型別
├── tests/                     # Vitest 測試（172 個）
├── docs/                      # PRD / SPEC / PLAN
...
| 4 | 📋 遠期 | In-App 自動更新（待 code signing） |
